...
local Settings = sigmalib.Settings
local Flags = sigmalib.Flags

local SetProps, SetChildren, InsertTheme, Create do
    InsertTheme = function(Instance, Type)
        table.insert(sigmalib.Instances, {
            Instance = Instance,
            Type = Type
        })
        return Instance
    ...
    local function Save(file)
        if readfile and isfile and isfile(file) then
            ...
            if type(decode) == "table" then
                if rawget(decode, "UISize") then sigmalib.Save["UISize"] = decode["UISize"] end
                if rawget(decode, "TabSize") then sigmalib.Save["TabSize"] = decode["TabSize"] end
                if rawget(decode, "Theme") and VerifyTheme(decode["Theme"]) then sigmalib.Save["Theme"] = decode["Theme"] end
            end
        end
    end
    
    pcall(Save, "Sigma library v8.1.json")
end
...
local Connections, Connection = {}, sigmalib.Connection do
...
local function VerifyTheme(Theme)
    for name,_ in pairs(sigmalib.Themes) do
        if name == Theme then
            return true
        end
    end
...
local Theme = sigmalib.Themes[sigmalib.Save.Theme]

local function AddEle(Name, Func)
    sigmalib.Elements[Name] = Func
end

local function Make(Ele, Instance, props, ...)
    local Element = sigmalib.Elements[Ele](Instance, props, ...)
    return Element
end
...
function sigmalib:GetIcon(index)
...
function sigmalib:SetTheme(NewTheme)
    if not VerifyTheme(NewTheme) then return end
    
    sigmalib.Save.Theme = NewTheme
    SaveJson("Sigma library v8.1.json", sigmalib.Save)
    
    local OldTheme = Theme
    Theme = sigmalib.Themes[NewTheme]
    
    if MainFrame then
        if NewTheme == "ScriptBoX" then
    ...
    for _, Val in pairs(sigmalib.Instances) do
    ...
    for _, TabData in pairs(sigmalib.Tabs) do
...
function sigmalib:SetScale(NewScale)
...
function sigmalib:MakeWindow(Configs)
    local WTitle = Configs[1] or Configs.Name or Configs.Title or "Sigma Library v8.1"
    local WMiniText = Configs[2] or Configs.SubTitle or "by : ScriptBoX"
    ...
    local UISizeX, UISizeY = unpack(sigmalib.Save.UISize)
    ...
    local MainScroll = InsertTheme(Create("ScrollingFrame", Components, {
        Size = UDim2.new(0, sigmalib.Save.TabSize, 1, -TopBar.Size.Y.Offset),
    ...
    ConnectSave(ControlSize1, function()
        if not Minimized then
            sigmalib.Save.UISize = {MainFrame.Size.X.Offset, MainFrame.Size.Y.Offset}
            SaveJson("Sigma library v8.1.json", sigmalib.Save)
        end
    end)
    
    ConnectSave(ControlSize2, function()
        sigmalib.Save.TabSize = MainScroll.Size.X.Offset
        SaveJson("Sigma library v8.1.json", sigmalib.Save)
    end)
    ...
    local function UpdateSearch(Query)
        ...
        for _, ElementData in pairs(sigmalib.AllElements) do
    ...
    function Window:SelectTab(TabSelect)
        if type(TabSelect) == "number" then
            sigmalib.Tabs[TabSelect].func:Enable()
        else
            for _,Tab in pairs(sigmalib.Tabs) do
    ...
    function Window:MakeTab(paste, Configs)
        ...
        local function Tabs()
            ...
            table.foreach(sigmalib.Tabs, function(_,Tab)
        ...
        FirstTab = true
        local Tab = {}
        table.insert(sigmalib.Tabs, {TabInfo = {Name = TName, Icon = TIcon}, func = Tab, Cont = Container})
        Tab.Cont = Container
        ...
        function Tab:AddSection(Configs)
            ...
            table.insert(sigmalib.AllElements, {
                Name = SectionName,
            ...
            local Section = {}
            table.insert(sigmalib.Options, {type = "Section", Name = SectionName, func = Section})
    ...
    task.spawn(function()
        ...
        ToggleGui.Name = "SigmaHub_Toggle_Circle"
        ...
        ToggleButton.Image = "rbxassetid://77394347412929"
...
local function CreateNotification(Icon, Title, Message, Duration)
    ...
    IconImage.Image = Icon or "rbxassetid://77394347412929"
    ...
    TitleLabel.Text = Title or "Sigma Hub"
...
function sigmalib:Notify(Configs)
    local Title = Configs.Title or Configs[1] or "Sigma Hub"
    local Message = Configs.Message or Configs[2] or Configs.Text or ""
    local Icon = Configs.Icon or "rbxassetid://77394347412929"
...
function sigmalib:SetUIBorderColor(color)
    if typeof(color) == "Color3" then
        self.Themes[self.Save.Theme]["UI Border Color"] = color
        if self.Save.Theme == sigmalib.Save.Theme then
...
function sigmalib:SetDiscordBorderColor(color)
    if typeof(color) == "Color3" then
...
function sigmalib:SetProfileBorderColor(color)
    if typeof(color) == "Color3" then
...
function sigmalib:SetHubColor(color)
    if typeof(color) == "Color3" then
...
function sigmalib:SetStrokeColor(color)
    if typeof(color) == "Color3" then
...
function sigmalib:SetTextColor(color)
    if typeof(color) == "Color3" then
...
function sigmalib:SetToggleOnColor(color)
    if typeof(color) == "Color3" then
...
function sigmalib:SetToggleOffColor(color)
    if typeof(color) == "Color3" then
...
function sigmalib:SetToggleKnobOnColor(color)
    if typeof(color) == "Color3" then
...
function sigmalib:SetToggleKnobOffColor(color)
    if typeof(color) == "Color3" then
...
function sigmalib:SetToggleBorderColor(color)
    if typeof(color) == "Color3" then
...
function sigmalib:SetBorderThickness(thickness)
...
function sigmalib:GetBorderColors()
    return {
...
function sigmalib:GetToggleColors()
    return {
...
function sigmalib:ResetBorderColors()
    local defaultTheme = self.Themes.ScriptBoX
...
function sigmalib:ResetToggleColors()
    local defaultTheme = self.Themes.ScriptBoX
...
task.spawn(function()
    ...
    sigmalib:Notify({
        Title = "Sigma Library",
        Message = "UI Sigma Library Download Completed",
        Duration = 10
    })
end)
